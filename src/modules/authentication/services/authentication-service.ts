...
class AuthenticationService {
    ...
    public async login(email: string, password: string): Promise<string> {
        ...
        const user = await db.pool.query(`SELECT first_name, last_name, password, email FROM users WHERE email=$1`, [email]);
    ...
    private async isSamePassword(password: string, encrypted: string): Promise<boolean> { 
        ...
        if(!encrypted || !password) return Promise.reject(false);

        return compare(password, encrypted);
    }

    public async checkUserCredentials(user: User, password: string) {
